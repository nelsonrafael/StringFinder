...
/**
 * Implementation of a text extractor from OOXML Excel files that uses SAX event
 * based parsing.
 */
public class XSSFEventBasedExcelExtractor extends POIXMLTextExtractor
	...
	/**
	 * Should we return the formula itself, and not the result it produces? Default
	 * is false
	 */
	...
	/**
	 * Concatenate text from &lt;rPh&gt; text elements in SharedStringsTable Default
	 * is true;
	 *
	 * @param concatenatePhoneticRuns true if runs should be concatenated, false
	 *                                otherwise
	 */
	 ...
	 */
	public void processSheet(SheetContentsHandler sheetContentsExtractor, Styles styles, Comments comments,
			SharedStrings strings, InputStream sheetInputStream) throws IOException, SAXException {
		...
		try {
			...
			ContentHandler handler = new XSSFSheetXMLHandler(styles, comments, strings, sheetContentsExtractor,
					formatter, formulasNotResults);
	...
	protected class SheetTextExtractor implements SheetContentsHandler {
		...
		/**
		 * Remove the delimiter if its found at the beginning of the text, or replace it
		 * with a tab if its in the middle.
		 */
		...
		/**
		 * Append the text for each header type in the same order they are appended in
		 * XSSFExcelExtractor.
		 *
		...
		/**
		 * Append the text for each footer type in the same order they are appended in
		 * XSSFExcelExtractor.
		 *
		 * @see XSSFExcelExtractor#getText()
		 * @see org.apache.poi.hssf.extractor.ExcelExtractor#_extractHeaderFooter(org.apache.poi.ss.usermodel.HeaderFooter)
